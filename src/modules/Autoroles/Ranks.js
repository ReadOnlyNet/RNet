...
class Ranks extends Base {
	...
	createRank(guild, role, name) {
		const guildConfig = this.rnet.guilds.get(guild.id);
		...
		return new Promise((resolve, reject) => {
			this.rnet.guilds.update(guild.id, { $set: { 'autoroles.ranks': guildConfig.autoroles.ranks } })
				.then(resolve)
				.catch(reject);
		});
	}
	...
	async deleteRank(guild, role) {
		const guildConfig = await this.rnet.guilds.getOrFetch(guild.id);
		if (!guildConfig) return Promise.reject('Error getting server configuration.');
	...
	async toggleRank(msg, rank) {
		const guildConfig = await this.rnet.guilds.getOrFetch(msg.channel.guild.id);
