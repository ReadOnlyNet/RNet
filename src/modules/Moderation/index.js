...
class Moderation extends Module {
	 ...
	 */
	start() {
		this.moderation = new Helper(this.config, this.rnet);
	...
	guildCreate(guild) {
		this.rnet.guilds.getOrFetch(guild.id)
		...
		models.Moderation.find({ server: guild.id, userid: member.id }).lean().exec()
			.then(docs => {
				for (let doc of docs) {
					...
					return member.addRole(role.id, encodeURIComponent('RNet role persist')).catch(() => false);
				}
			});
	}
