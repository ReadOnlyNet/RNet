...
class Music extends Module {
	...
	updateStats() {
		...
		redis.hsetAsync(`rnet:vc:${this.config.client.id}`, `${this.rnet.options.clusterId}:${this.config.state}`, playingConnections.length || 0)
			.catch(() => null);
	}
	...
	canCommand(msg, needsPerms) {
		let guildConfig = this.rnet.guilds.get(msg.channel.guild.id);
	 ...
	 */
	isCommander(msg) {
		const guildConfig = this.rnet.guilds.get(msg.channel.guild.id);
	...
	isBanned(search, guildConfig) {
		const globalConfig = this.rnet.globalConfig.music;
	...
	async toggleRepeat(msg) {
		const config = await this.rnet.guilds.getOrFetch(msg.channel.guild.id);

		config.music = config.music || {};
		config.music.repeat = !config.music.repeat;

		try {
			await this.rnet.guilds.update(msg.channel.guild.id, { $set: { 'music.repeat': config.music.repeat } });
		} catch (err) {
			this.logger.error(err, {
	...
	async getInfo(url, guildId, fetchAll) {
		...
		if (volume !== 1) {
			let guildConfig = this.rnet.guilds.get(guildId);
