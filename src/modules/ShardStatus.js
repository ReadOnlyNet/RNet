...
const { Collection, Module } = require('@rnet.cf/rnet-core');
...
class ShardStatus extends Module {
	constructor(...args) {
		...
		this.description = 'RNet core module.';
		this.core = true;
		this.list = false;
		this.enabled = true;
		this.hasPartial = false;

		this.cmClient = this.rnet.cmClient;
	}
	 ...
	 */
	shardReady(id) {
		this.logger.info(`[RNet] Shard ${id} ready.`);
	 ...
	 */
	shardResume(id) {
		this.logger.info(`[RNet] Shard ${id} resumed.`);
	...
	async postStat(key) {
		...
		this.prom.register.getSingleMetric('rnet_app_discord_shard').inc({ type: key });
