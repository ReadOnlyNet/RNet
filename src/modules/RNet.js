...
class RNet extends Module {
	...
	unload() {
		...
		this.rnet.commands.removeListener('command', this.commandSuccessListener);
		this.rnet.commands.removeListener('command', this.commandFailureListener);
	...
	async setMessage(key, id, msg) {
		key = `rnet.${key}`;
		try {
			await this.rnet.guilds.update(id, { $set: { [key]: msg ? msg.id : null } });
	...
	async logEvent(event) {
		try {
			...
			each(docs, async doc => {
				...
				if (!doc.rnet || !doc.rnet.logChannel || !doc.rnet.logEnabled) return;

				const channel = this.client.getChannel(doc.rnet.logChannel);
	...
	async logGuildEvent(event, guild) {
		...
		if (guild.memberCount && guild.memberCount >= 250) {
			...
			if (!this.guildlogChannel) {
				this.guildlogChannel = await this.rnet.restClient.getRESTChannel('341532519306362881').catch(() => null);
			}

			if (this.guildlogChannel) {
				let guildConfig = await this.rnet.guilds.getOrFetch(guild.id);
				if (guildConfig) {
	...
	async logDirectMessage(message) {
		try {
			...
			each(docs, async doc => {
				...
				if (!doc.rnet || !doc.rnet.dmEnabled || !doc.rnet.dmChannel) return;

				const channel = this.client.getChannel(doc.rnet.dmChannel);
			...
			this.logger.error(err, { type: 'rnet.logDirectMessage' });
		}
	}
	...
	async updateStats() {
		...
		if (!this.statsChannel) {
			try {
				...
				return this.logger.error(err, { type: 'rnet.updateStats.createChannel' });
			}
		}
	...
	async sendStats(guild, shards, vcs, ffmpegs) {
		...
		if (!message) {
			...
			}).catch(err => this.logger.error(err, { type: 'rnet.sendStats.sendMessage' }));
		}

		message.edit({ embed }).catch(err => this.logger.error(err, { type: 'rnet.sendStats.editMessage' }));
	}
}
