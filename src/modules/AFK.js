...
class AFK extends Module {
	...
	async setAFK(msg, status) {
		...
		this.rnet.guilds.update(msg.channel.guild.id, { $set: { 'afk.users': guildConfig.afk.users } })
			.catch(err => this.logger.error(err));
	...
	ignoreChannel(message, guildConfig) {
		...
		return new Promise((resolve, reject) =>
			this.rnet.guilds.update(message.channel.guild.id, { $set: { 'afk.ignoredChannels': guildConfig.afk.ignoredChannels } })
