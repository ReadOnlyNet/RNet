...
class Debug extends Command {
	...
	permissionsFn({ message }) {
		if (message.guild.id === this.config.rnetGuild) return true;
	...
	async toggleDebug(guildId, value, host = 'premium.rnet.cf') {
		try {
			const options = {
				method: 'POST',
	...
	async execute({ message, args }) {
		...
		try {
			await this.toggleDebug(guildId, true, 'premium.rnet.cf');
			await this.toggleDebug(guildId, true, 'staff.rnet.cf');
