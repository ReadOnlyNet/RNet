...
class Data extends Command {
	...
	async guild({ message, args }) {
		const clientOptions = this.rnet.clientOptions;
		...
		embed.fields.push({ name: '\u200b', value: `[Dashboard](https://www.rnet.cf/server/${guild._id})`, inline: true });
	...
	async user({ message, args }) {
		...
		if (!resolvedUser) {
			resolvedUser = await this.rnet.restClient.getRESTUser(args[0]).catch(() => false);
		}
	...
	addmodule({ message, args }) {
		if (!this.rnet.modules.has(args[0])) return this.error(message.channel, `That module does not exist.`);
	...
	async shards({ message, args }) {
		...
		try {
			const instanceStats = await Promise.all([
				axios.get('http://prod01.rnet.lan:5000/shards'),
				axios.get('http://prod02.rnet.lan:5000/shards'),
				axios.get('http://prod03.rnet.lan:5000/shards'),
			]);
			// const shardStats = await this.rnet.ipc.awaitResponse('shards');
	...
	async associate({ message, args }) {
		let associates = this.rnet.globalConfig.associates || [];
