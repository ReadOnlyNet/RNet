...
class Youtube extends Command {
	 ...
	 */
	execute({ message }) {
		this._music = this.rnet.modules.get('Music');
	...
	async add(message) {
		...
		if (await music.isStreamLimited(message.guild)) {
			return this.sendMessage(message.channel, [
				`**Sorry, our music server is currently at peak capacity <:RNetSweats:342841357104316416>**`,
				...
				.catch(err => {
					if (err === '20m') {
						...
						if (this.isServerMod(message.member, message.channel)) {
							msgArray.push(`You can upgrade RNet to enable this feature and more at <${this.config.site.host}/upgrade>`);
						}
					...
					if (err === '60m') {
						if (this.isServerMod(message.member, message.channel)) {
							return this.sendMessage(message.channel, [
								...
								`You can upgrade RNet to enable this feature and more at <${this.config.site.host}/upgrade>`
								]);
						}
