...
class Stats extends Command {
	...
	async execute({ message, args }) {
		...
		const [shards, guildCounts, vc, ffmpegs] = await Promise.all([
			redis.hgetallAsync(`rnet:stats:${state}`),
			redis.hgetallAsync(`rnet:guilds:${this.config.client.id}`),
			redis.hgetallAsync(`rnet:vc`), // eslint-disable-line
			this.getFFmpegs(),
		]).catch(() => false);
		...
		let streams = this.config.isCore ? data.allConnections : `${data.playing}/${data.voice}`,
			...
			footer = `${stateName} | Cluster ${this.rnet.options.clusterId || this.rnet.options.shardId} | Shard ${message.channel.guild.shard.id}`;
