...
class Diagnose extends Command {
	...
	async execute(e) {
		...
		if (message.guild.id === this.config.rnetGuild && args.length > 1 && args[args.length - 1].match(/([0-9]+)/)) {
			if (this.isServerMod(message.member, message.guild.defaultChannel)) {
				let guildConfig = await this.rnet.guilds.getOrFetch(args.pop());
				if (!guildConfig) {
		...
		const command = this.rnet.commands.find(c => c.name === args[0] && c.permissions !== 'admin');

		if (command) {
			return this.diagnoseCommand(command, e);
		}

		const module = this.rnet.modules.find(m => !m.core &&
