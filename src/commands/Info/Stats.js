...
class Stats extends Command {
	...
	async execute({ message, args }) {
		...
		const [shards, guildCounts, vc] = await Promise.all([
			this.redis.hgetall(`rnet:cstats:${this.config.client.id}`),
			this.redis.hgetall(`rnet:guilds:${this.config.client.id}`),
			this.redis.hgetall(`rnet:vc`), // eslint-disable-line
		]).catch(() => false);
		...
		let streams = this.config.isCore ? data.allConnections : `${data.playing}/${data.voice}`,
			...
			footer = `PID ${process.pid} | ${stateName} | Cluster ${this.rnet.clientOptions.clusterId.toString()} | Shard ${message.channel.guild.shard.id}`;
