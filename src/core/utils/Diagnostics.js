class Diagnostics {
	...
	async diagnose(guildId, name) {
		const guild = this.rnet.client.guilds.get(guildId);
		const guildConfig = await this.rnet.guilds.getOrFetch(guildId);
		...
		const command = this.rnet.commands.find((c) => c.name === name && c.permissions !== 'admin');

		if (command) {
			return this.diagnoseCommand(command, guild, guildConfig);
		}

		const mod = this.rnet.modules.find((m) => !m.core &&
	...
	diagnoseDefault(guild, guildConfig, perms) {
		...
		const clientMember = guild.members.get(this.rnet.client.user.id);
		...
		if (!clientMember.roles || !clientMember.roles.length) {
			...
			diagnosis.issues.push(`You can fix this by authorizing the bot here: https://rnet.cf/invite`);
		}

		for (const perm of permissions) {
			if (!clientMember.permission.has(perm)) {
				const permission = this.rnet.config.permissionsMap[perm];
		...
		if (perms && clientMember.roles && clientMember.roles.length) {
			const highestRole = this.rnet.utils.highestRole(guild, clientMember);
	...
	diagnoseCommand(command, guild, guildConfig) {
		...
		if (diagnosis.issues.length) {
			embed.color = this.rnet.utils.getColor('orange');
			embed.fields.push({ name: 'Issues', value: diagnosis.issues.join('\n'), inline: false });
		} else {
			embed.color = this.rnet.utils.getColor('green');
	...
	diagnoseModule(module, guild, guildConfig) {
		...
		const embed = {
			...
			footer: {
				...
				text: `${this.rnet.config.stateName} | Cluster ${this.rnet.options.clusterId} | Shard ${guild.shard.id} | ID ${guild.id}`,
			},
		};
