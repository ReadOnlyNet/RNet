...
function initMatomo(rnet) {
    ...
    setInterval(() => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
        if (!rnet.isReady) { return; }
    ...
    setInterval(() => {
        ...
        rnet.players.forEach((p) => {
            ...
            musicBuffer.guildBuffer.push({
                token_auth: rnet.globalConfig.matomoTokenAuth || 'ee9e3342c6a8f02ea0e1278060dd3db5',
    ...
    rnet.internalEvents.on('music', ({ type, guild, channel, user, search, trackInfo }) => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
        let guildEvent;
        let userEvent;
        let player;
        ...
        switch (type) {
            case 'start':
                guildEvent = {
                    ...
                    _cvar: JSON.stringify({
                        1: ['Guild ID', guild.id],
                        2: ['Server', rnet.config.stateName],
                    }),
                };

                player = rnet.players.get(guild.id);
                    ...
                    player.voiceChannel.voiceMembers.forEach((m) => {
                        musicBuffer.userBuffer.push({
                            ...
                            _cvar: JSON.stringify({
                                1: ['Guild ID', guild.id],
                                2: ['User ID', m.id],
                                3: ['Server', rnet.config.stateName],
                            }),
                        });
                    });
                }
    ...
    rnet.internalEvents.on('actionlog', ({ type, guild }) => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
    ...
    rnet.internalEvents.on('autoresponder', ({ type, guild }) => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
        if (!matomoUsers || !matomoGuilds) {
            return;
        }
    ...
    rnet.internalEvents.on('automod', ({ type, guild }) => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
        if (!matomoUsers || !matomoGuilds) {
            return;
        }
    ...
    rnet.commands.on('command', ({ command, message, guildConfig, args, time, isServerAdmin, isServerMod }) => {
        if (rnet.globalConfig && rnet.globalConfig.enableMatomo && rnet.globalConfig.enableMatomo[rnet.config.stateName.toLowerCase()]) { return; }
        if (!matomoUsers || !matomoGuilds) {
            return;
        }
        ...
        commandBuffer.userBuffer.push({
            ...
            _cvar: JSON.stringify({
                ...
                3: ['Guild ID', guild.id],
                4: ['Server', rnet.config.stateName],
                5: ['User ID', user.id],
            }),
