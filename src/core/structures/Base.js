...
class Base {
	 ...
	 */
	get globalConfig() {
		return this._rnet.globalConfig;
	}
	...
	async getStreamCount() {
		try {
			var vcs = await redis.hgetallAsync(`rnet:vc:${this.config.client.id}`);
		} catch (err) {
	...
	async sendWebhook(channel, options, guildConfig) {
		...
		let payload = Object.assign({}, this.rnet.webhooks.default);
		...
		try {
			var webhook = await this.rnet.webhooks.getOrCreate(channel);
		} catch (err) {
