...
class GuildCollection extends Collection {
	 ...
	 * @param {Object} config The RNet configuration object
	 * @param {RNet} rnet The RNet instance
	 */
	constructor(config, rnet) {
		super();
	...
	registerGuild(guild, newGuild) {
		...
		return new Promise((resolve, reject) => {
			// add modules
			for (let mod of this.rnet.modules.values()) {
			...
			for (let cmd of this.rnet.commands.values()) {
				if (cmd.permissions === 'admin') continue;

				// ignore commands that belong to a module
				if (this.rnet.modules.find(o => o.module === cmd.group) && doc.modules[cmd.group] === false) {
	...
	dmOwner(guild) {
		...
		msgArray.push('**2.** Setup the bot at **https://www.rnet.cf**');
		msgArray.push('**3.** Commands do not work in DM.');
		msgArray.push(`**4.** Join the RNet discord server for questions, suggestions, or updates. **https://www.rnet.cf/discord**`);
