...
class RPCManager extends EventEmitter {
	...
	constructor(rnet) {
		...
		this.clusterConfig = rnet.clientOptions;
		...
		this.nats = Nats.connect({ url: 'nats://ares.rnet.cf:4222' });
	...
	onMessage(message) {
		...
		try {
			return command(this.rnet, this.config, message);
		} catch (err) {
